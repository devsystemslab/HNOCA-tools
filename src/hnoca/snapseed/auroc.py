...
from .utils import dict_to_binary, frac_nonzero, get_expr, jit_frac_nonzero, masked_max, masked_mean
...
def annotate_snap(
    ...
    Args:
        ...
        auc_weight: Weight to give to AUROC in the final score
        expr_weight: Weight to give to detection rate in the final score
        marker_summary_fun: Function to use to summarize over markers for the same
            annotation. Options are "max" and "mean".
        apply_fun: Function to use for applying the auroc function. Options are "vmap"
            and "numpy". vmap is a lot faster but can cause OOM errors for big matrices.
    """
    ...
    metrics = auc_expr(adata, group_name, features=features, layer=layer, apply_fun=apply_fun)
    ...
    if marker_summary_fun == "max":
        ...
        raise ValueError(f"Invalid marker_summary_fun: {marker_summary_fun}. Current options are 'max' and 'mean'.")
    # Combine metrics
    assignment_scores = (auc_weight * auc_max + expr_weight * expr_max) / (auc_weight + expr_weight)
...
@jax.jit
def jit_auroc(x, groups):  # noqa D103
...
def numpy_auroc(x, groups):  # noqa D103
