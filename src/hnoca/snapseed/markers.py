...
def find_markers(
    ...
    expr_df = matrix_to_long_df(metrics["auroc"], features=metrics["features"], groups=metrics["groups"]).rename(
        columns={"value": "auroc"}
    )

    auc_df = matrix_to_long_df(metrics["frac_nonzero"], features=metrics["features"], groups=metrics["groups"]).rename(
        columns={"value": "frac_nonzero"}
    )
    ...
    marker_df = expr_df.merge(auc_df, on=["group", "feature"]).merge(out_expr_df, on=["group", "feature"])
    # Compute detection ratio
    marker_df["detection_ratio"] = marker_df["frac_nonzero"] / marker_df["frac_nonzero_out"]
