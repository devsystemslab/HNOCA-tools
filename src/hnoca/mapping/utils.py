...
def prepare_features(query_adata, ref_model):
    ...
    extra_X = sparse.csr_matrix((new_adata.shape[0], len(missing_features)), dtype=new_adata.X.dtype)
    ...
    new_var = pd.DataFrame({"symbol": list(new_adata.var_names) + list(missing_features)}).set_axis(
        list(new_adata.var_names) + list(missing_features)
    )
