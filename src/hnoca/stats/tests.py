...
def anova(e, group, covar=None, return_coef_group=None):  # noqa D103
    if issparse(e):
        e = e.todense()
...
def ancova_group_test(expr, group, covar=None, num_threads=1, return_coef_group=None, var_names=None):  # noqa D103
    ...
    if num_cores > 1:
        results = Parallel(n_jobs=num_cores)(
            delayed_anova(expr[:, i], group, covar, return_coef_group) for i in range(expr.shape[1])
        )
    else:
        results = []
...
def f_nonzero(e, covar=None):  # noqa D103
    if issparse(e):
        e = e.todense()
    e = np.array(e).flatten()
...
def f_nonzero_test(expr, covar=None, num_threads=1, var_names=None):  # noqa D103
    if covar is not None:
        covar = pd.DataFrame(covar)

    num_cores = min(num_threads, multiprocessing.cpu_count())
    if num_cores > 1:
        results = Parallel(n_jobs=num_cores)(delayed_f_nonzero(expr[:, i], covar) for i in range(expr.shape[1]))
    else:
        results = []
        for i in tqdm(range(expr.shape[1])):
